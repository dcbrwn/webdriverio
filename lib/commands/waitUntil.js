...
export default function (condition, timeout, timeoutMsg, interval) {
    ...
    return timer.catch((e) => {
        ...
        if (message === 'timeout' && typeof timeoutMsg === 'string') {
            throw new WaitUntilTimeoutError(timeoutMsg)
        }

        if (e.type === 'NoSuchElement') {
            throw new WaitUntilTimeoutError(message)
        }

        throw new WaitUntilTimeoutError(`Promise was rejected with the following reason: ${message}`)
    })
}
